...
ggplot(data=kmeans12_centers, aes(x=rownames(kmeans12_centers), y=Deaths, fill=rownames(kmeans12_centers))) +
...
centers <- as.data.frame(t(kmeans12_centers))

c1 <- subset(kmeans12_centers, rownames(kmeans12_centers) == 1)
c2 <-  subset(kmeans12_centers, rownames(kmeans12_centers) == 2)

pivot_c2 <- c2 %>% 
  pivot_longer(cols = colnames(c2))

pivot_c1 <- c1 %>% 
  pivot_longer(cols = colnames(c1))


library(bbplot)
 ggplot()+ 
  geom_bar(data = pivot_c2, aes(x = name, y = value, position = "dodge2"),stat = "identity", fill = "red", alpha = 0.7, col = "red")+
  theme(axis.text = element_text(size = 8, angle = 90, vjust = 1, hjust=1))+
   geom_bar(data = pivot_c1, aes(x = name, y = value, position = "dodge2"),stat = "identity", fill = "steelblue", alpha = 0.7,  col = "steelblue") 
   
library(bbplot)
ggplot()+ 
  geom_bar(data = centers, aes(x = name, y = value, fill = rownames(centers))
           
           
           ,stat = "identity", fill = "red", alpha = 0.7, col = "red")+
  theme(axis.text = element_text(size = 8, angle = 90, vjust = 1, hjust=1))+
   geom_bar(data = pivot_c1, aes(x = name, y = value),stat = "identity", fill = "steelblue", alpha = 0.7,  col = "steelblue")+   
...
k2 <- kmeans(combined_kmeans1, centers = 3, nstart = 25)
paste0('Within cluster variation of data: ',round(k2$tot.withinss,2))
combined_kmeans1$cluster=as.factor(k2$cluster)
kmeans13_centers <- data.frame(k2$centers)

write.csv(kmeans13_centers,"three_cluster.csv", row.names = TRUE)

#comparing cluster centers 
ggplot(data=kmeans13_centers, aes(x=rownames(kmeans13_centers), y=Obesity, fill=rownames(kmeans13_centers))) +
    geom_bar(stat="identity")


ggplot(data=kmeans13_centers, aes(x=rownames(kmeans13_centers), y=Deaths, fill=rownames(kmeans13_centers))) +
    geom_bar(stat="identity")


ggplot(data=kmeans13_centers, aes(x=rownames(kmeans13_centers), y=Alcoholic.Beverages, fill=rownames(kmeans13_centers))) +
    geom_bar(stat="identity")
...
k3 <- kmeans(combined_kmeans2, centers = 2, nstart = 25)
paste0('Within cluster variation of data: ',round(k3$tot.withinss,2))
combined_kmeans2$cluster=as.factor(k3$cluster)
kmeans22_centers <- k3$centers
